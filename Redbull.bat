...
timeout /t 2 /nobreak >nul
echo Welcome to the Redbull Panel v5.0.0(beta)
...
:startssh
echo Do you want to:
echo 1. SSH tunnel an iPhone (must be jailbroken with OpenSSH)
echo 2. SSH tunnel another device (computer, etc)
set /p x=
IF %x%==1 start menucache\redbull-menus\crackiphone.bat
IF %x%==2 start menucache\redbull-menus\sshlogin.bat
goto r
